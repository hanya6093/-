...
main()
{
	...
	num=VCI_FindUsbDevice2(pInfo1); // 找设备 pinfo1中含有usb-can接口信息 返回接口数量
	// 打印设备信息
	...
	int i=0;
	for(i = 0; i < send[0].DataLen; i++) // 填充数据
	{
		send[0].Data[i] = i;
	}
	...
	int ret;
	ret=pthread_create(&threadid,NULL,receive_func,&m_run0); // 创建线程 并启动线程函数
	// 发送消息 5次 每个CAN口一次
	int times = 5;
	while(times--)
	{
